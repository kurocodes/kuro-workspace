...
export default function ImageMarquee({ images, speed = 50 }: ImageMarqueeProps) {
    ...
    <div className="w-full overflow-hidden">
      <motion.div className="flex w-max" style={{ x }}>
        ...
        <div ref={contentRef} className="flex shrink-0">
          {images.map((src, i) => (
            <img key={`a-${i}`} src={src} className="h-18 w-auto" />
          ))}
        </div>
        ...
        <div className="flex shrink-0">
          {images.map((src, i) => (
            <img key={`b-${i}`} src={src} className="h-18 w-auto" />
          ))}
        </div>
      </motion.div>
    </div>
